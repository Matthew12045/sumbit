...
- **Summarization:** the user's own gateway via the Anthropic-compatible API — `anthropic` SDK with `base_url=https://gateway.9arm.co`, `auth_token` (`ANTHROPIC_AUTH_TOKEN`), model `qwen3.6-35b-a3b`. **Do not append `/v1` to the base URL** (the SDK adds it). The call is a **blocking** `messages.create()` — a raw `tools/probe_stream.py` run (2026-08-10) showed the gateway buffers the whole completion server-side (no `thinking_delta` events during the silent thinking phase; the text arrives in a sub-second burst at the end), so streaming adds no early-abort value and `SUMMARIZE_TIMEOUT_SECONDS` (180) is the ceiling. The completed output is checked post-hoc for an exact-repeat loop (`REPETITION_WINDOW_CHARS=300`, `REPETITION_MIN_REPEATS=3`); a loop raises `StalledGenerationError(progressed=True)` and posts a visible ⚠️ note via `bot.py`. Retry policy: retry once **only** on `APITimeoutError` — `EmptySummaryError` and the loop error are never retried, since re-running temperature=0 against the same reasoning trace would reproduce them. Budget defaults: `MAX_PROMPT_CHARS=48000` (128k context, conservative ~1.5 chars/token), `SUMMARY_MAX_TOKENS=8192` (richer schema + qwen thinking). The output schema carries context, not labels: `overview`, `topics` (`title`+`detail`), `decisions` (`decision`+`rationale`), `action_items` (`action`+`owner`+`due`), `open_questions`. An empty completion (`EmptySummaryError`) also posts a visible ⚠️ note via `bot.py`.

**Bot layout** (`meeting_bot/`): `config` (`.env` → frozen dataclass + `doctor()`), `audio` (48k-stereo-int16 → 16k-mono-f32 numpy resample), `chunker` (silence segmentation), `sink` (py-cord per-user capture), `transcriber` (single worker thread → mlx-whisper), `transcript` (speaker/time events), `summary_parse` (robust parse), `summarizer` (blocking gateway call), `poster` (embed + markdown attachment), `bot` (`MeetingBot`, auto-trigger on voice-channel-empty), `main`/`__main__`. **Import rule:** `config`/`audio`/`chunker`/`transcript`/`summary_parse` import only stdlib + numpy at module scope, so the jailed agent can smoke-import them and `pytest` runs without Discord/MLX/anthropic.
