...
- **STT:** local `mlx-whisper` (`mlx-community/whisper-large-v3-mlx`, `language="th"`) — free, Metal-accelerated on Apple Silicon; shares the GPU with the gateway stack. Default is the **non-turbo** large-v3 because large-v3-turbo enters confident repetition-loop hallucinations on Thai (the 2026-08-09 empty-summary bug); turbo is selectable via `WHISPER_MODEL`. `transcriber.py` hardens the decode (`condition_on_previous_text=False`, T=0 greedy) and re-decodes once with a small temperature bump + Thai preamble when a decode is flagged garbage — watch for `recovered after anti-loop retry` vs `still garbage after anti-loop retry` in the logs.
